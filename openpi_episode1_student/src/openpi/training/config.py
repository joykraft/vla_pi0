...
import openpi.policies.libero_policy_two as libero_policy_two
...
class DataConfig:
    ...
    # Root directory for the dataset, if not using the default location.
    root: str | None = None
...
class LeRobotLiberoDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # print action_dim and model_type
        # print("-------enpei test single arm-------")
        # print(model_config.action_dim)
...
@dataclasses.dataclass(frozen=True)
class LeRobotLiberoDataConfigTwoArms(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # print action_dim and model_type
        # print("-------enpei test dual arms-------")
        # print(model_config.action_dim)
        # print(model_config.model_type)

        data_transforms = _transforms.Group(
            inputs=[libero_policy_two.LiberoInputsTwo(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[libero_policy_two.LiberoOutputsTwo()],
        )
...
_CONFIGS = [
    ...
    # 单臂配置
    TrainConfig(
        name="enpei_robot_demo_drawer_low_mem_finetune",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotLiberoDataConfig(
            repo_id="enpeicv/demo_drawer_openpi", # 数据集repo_od
            root="./enpei_dataset/demo_drawer_openpi", # 数据集路径
            base_config=DataConfig(prompt_from_task=True),
        ),
